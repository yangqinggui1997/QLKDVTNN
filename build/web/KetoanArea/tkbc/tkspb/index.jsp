...
if(manguoidung != null){
    if(manguoidung.substring(0,4).equals("NDQT") || manguoidung.substring(0,4).equals("NDBH") || manguoidung.substring(0,4).equals("NDTK")){
        ...
        %>
<% request.setAttribute("daughter", "daughter"); request.setAttribute("tkspb", "Yang - Thống kê sản phẩm bán"); %>
<jsp:include page="../../../View/top.jsp"></jsp:include>
                    ...
                    <table class="table table-hover" id="tbltk">   
                        ...
                        <% String ac=request.getParameter("action");
                                if(ac != null){  
                                    if(keyse != ""){
                                        for (sanphamban spb : sanphambanBUS.TKSPB(keyse)) {
                        %>
                        <tr>
                            <td hidden=""><%= spb.getIdSPB()%></td>
                            <% nhanvien nv=nhanvienBUS.getNV(spb.getIdNV()); %>
                            <td><%= nv.getTenNV() %></td>
                            <td><%= String.format("%,d",spb.getTSLBanNgay())%></td>
                            <td><%= CommonCommand.dedateformat(spb.getNgayTK().toString()) %></td>
                            <td><%= CommonCommand.dedateformat(spb.getNgayCN().toString()) %></td>
                        </tr>
                        <% }} else{ %>
